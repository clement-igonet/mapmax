...
//
// Ground ARROWS are rendered INSIDE the photosphere's WebGL layer (see the
// plugin's setNavArrows / groundPick): drawn on the floor plane they cover the
// whole viewport and are never clipped by MapLibre's map-plane near clip at
// grazing pitch — the "cropped arrow" is impossible there (#26).
//
// Nearby 360° photospheres are shown as ground DOTS (a MapLibre circle layer),
// hit-tested in screen space via map.project (queryRenderedFeatures is
// unreliable at eye-level pitch).
...
export function setupNavigation(map) {
  ...
  map.on('click', (e) => {
    ...
    const id = hitAt(map, e.point);
    if (id) go(map, id);
  });

  map.on('mousemove', (e) => {
    if (!isStreetMode()) return;
    map.getCanvas().style.cursor = hitAt(map, e.point) ? 'pointer' : '';
  });
}

// The clickable target under `point`: a ground arrow (ray-cast in the plugin)
// first, then the nearest POI dot in screen space.
function hitAt(map, point) {
  const arrowId = _photosphere()?.groundPick?.(point.x, point.y);
  if (arrowId) return arrowId;
  let best = null;
  let bestD = HIT_PX;
  for (const p of pois) {
    const s = map.project(p.lngLat);
...
async function refresh(map, pic) {
  ...
  _photosphere()?.setNavArrows?.(arrows.map((a) => ({ bearing: a.bearing, id: a.targetId })));
...
function clearNav(map) {
  arrows = [];
  pois = [];
  _photosphere()?.setNavArrows?.([]);
  map.getSource(POI_SRC)?.setData(EMPTY);
  map.getCanvas().style.cursor = '';
}

function ensurePoiLayer(map) {
