// The pose-editor gesture algebra: compose "grab the photo" drags onto a
// capture pose and re-extract yaw/pitch/roll for the Panoramax PATCH. Pure and
// self-contained — it mirrors maplibre-gl-photosphere's rendering conventions
// (world x = east, y = north, z = up; azimuth from +y toward +x; column-major
// mat3 with rows right/forward/up) without depending on it, so this package
// stays viewer-agnostic. Feed the result to `setPanoPose()` on the viewer and
// to `posePatchRequest()` here.
//
// Editing lives HERE by design: a correction only makes sense against the
// data source that stores it (Panoramax). Viewers render; this edits.

import { normalizeYaw } from './edit.js';

// World → camera-frame rotation of a capture pose (degrees), column-major.
...
// Apply a pose matrix to a world direction (three dot products).
...
// Compose a view-space editing gesture onto a pose. `camera` = {yawDeg,
// pitchDeg} of the CURRENT look direction (the axes the user reasons in);
// deltas in degrees:
