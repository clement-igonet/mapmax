...
const chrome = await page.evaluate(() => ({
  ...
  // #124: the level switch exists and stays hidden while no other level is near.
  levelSwitchIdle: document.getElementById('level-switch')?.hidden === true,
}));
...
assert.ok(chrome.levelSwitchIdle, 'level switch must exist and start hidden (#124)');
