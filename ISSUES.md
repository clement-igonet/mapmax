...
|---|---|---|---|
| [#144](https://github.com/clement-igonet/mapmax/issues/144) | Manual rotation pad (pitch up/down, roll left/right) | [#151](https://github.com/clement-igonet/mapmax/pull/151) | ▲▼ pitch, ◀▶ roll, centre levels; 0.5°/click, 2° with ⇧, clamped ±90°; one small grid chip (the shape that survives #100); also brings the position cluster to the right column (fix stranded on the parked #142 branch). On [sandbox](https://sandbox.mapmax.confinia.io), e2e drives the pad in a real browser |
| [#146](https://github.com/clement-igonet/mapmax/issues/146) | Sandbox branch deploys bounce production | [#149](https://github.com/clement-igonet/mapmax/pull/149) merged: sandbox owns `mapmax-sandbox-stack.service` (edge-sandbox + web-sandbox); production unit stops `down`-ing the whole project and drops its sandbox dependency; sandbox deploys restart only their unit and main deploys no longer rebuild web-sandbox. Verified: bouncing the sandbox keeps www at 200 throughout, and a main deploy left the #142 branch intact on sandbox; closed 2026-08-16 |
| [#142](https://github.com/clement-igonet/mapmax/issues/142) | Auto-fix panorama orientation against the vector world | [#143](https://github.com/clement-igonet/mapmax/pull/143) (draft) | **PARKED — not for promotion**: works end-to-end (per-axis Yaw/Pitch/Roll from a spin-scan of the vector world; Pitch/Roll locked until the yaw is aligned or confirmed by eye), but too rough to ship — remaining: slow scan, accuracy unvalidated at scale, no sequence consensus, overlay soak-testing. SOLVED on the branch: tilt now measurable without a roofline (façade vertical correlation feeds the same fit) and the #144 pad is merged in as the manual fallback. Known issues listed on the PR; sandbox runs the branch |
...
| Issue | Title | Notes |
|---|---|---|
| [#139](https://github.com/clement-igonet/mapmax/issues/139) | Native flat projection (photosphere 0.5.0) replaces the flatpatch canvas | gnomonic-correct, map visible outside the photo window; retires src/flatpatch.js |
...
|---|---|---|
| [#125](https://github.com/clement-igonet/mapmax/issues/125) | Legend chip color ≠ dot colors | [#127](https://github.com/clement-igonet/mapmax/pull/127) merged: each chip previews BOTH dot swatches (360° + flat) straight from the adapter's `dotColors` — chip now teaches the real mapping; closed 2026-08-15 |
...
| [#110](https://github.com/clement-igonet/mapmax/issues/110) | Graduate vendored photosphere improvements to maplibre-gl-photosphere | [maplibre-gl-photosphere v0.4.0](https://github.com/clement-igonet/maplibre-gl-photosphere/releases/tag/v0.4.0) released canonically (bump PR → npm + tag + GH Release, [PR #114](https://github.com/clement-igonet/mapmax/pull/114) un-forks MapMax onto the verbatim release files + VENDOR.md); gesture/edit vendored from [maplibre-gl-panoramax](https://github.com/clement-igonet/maplibre-gl-panoramax)@main; **photosphere [v0.5.0](https://github.com/clement-igonet/maplibre-gl-photosphere/releases/tag/v0.5.0) released canonically 2026-08-15** (per-dot POI colors + native flat projection) and re-pinned verbatim via [#138](https://github.com/clement-igonet/mapmax/pull/138); closed 2026-08-12 |
